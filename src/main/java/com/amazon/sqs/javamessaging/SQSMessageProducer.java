...
import com.amazon.sqs.javamessaging.message.JMSMessagePropertyValue;
...
/**
 * A client uses a MessageProducer object to send messages to a queue destination. A MessageProducer
 * object is created by passing a Destination object to a message-producer creation method supplied
 * by a session.
 * <P>
 * A client also has the option of creating a message producer without supplying a queue
...
public class SQSMessageProducer implements MessageProducer, QueueSender {
  ...
  /**
   * State of MessageProducer. True if MessageProducer is closed.
   */
  ...
  void sendInternal(SQSQueueDestination queue, Message rawMessage) throws JMSException {
    ...
    Map<String, MessageAttributeValue> messageAttributes =
        propertyToMessageAttribute((SQSMessage) message);

    /**
     * These will override existing attributes if they exist. Everything that has prefix JMS_ is
     * reserved for JMS Provider, but if the user sets that attribute, it will be overwritten.
     */
    ...
    SendMessageRequest sendMessageRequest =
        new SendMessageRequest(queue.getQueueUrl(), sqsMessageBody);
    sendMessageRequest.setMessageAttributes(messageAttributes);

    // for FIFO queues, we have to specify both MessageGroupId, which we obtain from standard
    // property JMSX_GROUP_ID
    // and MessageDeduplicationId, which we obtain from a custom provider specific property
    // JMS_SQS_DEDUPLICATION_ID
    // notice that this code does not validate if the values are actually set by the JMS user
    // this means that failure to provide the required values will fail server side and throw a
    // JMSException
    if (queue.isFifo()) {
  ...
  /**
   * Not verified on the client side, but SQS Attribute names must be valid letter or digit on the
   * basic multilingual plane in addition to allowing '_', '-' and '.'. No component of an attribute
   * name may be empty, thus an attribute name may neither start nor end in '.'. And it may not
   * contain "..".
   */
  Map<String, MessageAttributeValue> propertyToMessageAttribute(SQSMessage message)
    ...
    while (propertyNames.hasMoreElements()) {
      ...
      messageAttributeValue.setDataType(propertyObject.getType().getDataType());
      messageAttributeValue.setStringValue(propertyObject.getStringValue());
   ...
   */
  private void addReplyToQueueReservedAttributes(
      Map<String, MessageAttributeValue> messageAttributes,
    ...
    if (replyTo instanceof SQSQueueDestination) {
      ...
      /**
       * This will override the existing attributes if exists. Everything that has prefix JMS_ is
       * reserved for JMS Provider, but if the user sets that attribute, it will be overwritten.
       */
      addStringAttribute(messageAttributes, SQSMessage.JMS_SQS_REPLY_TO_QUEUE_NAME,
          replyToQueue.getQueueName());
      addStringAttribute(messageAttributes, SQSMessage.JMS_SQS_REPLY_TO_QUEUE_URL,
          replyToQueue.getQueueUrl());
    }
  }
   ...
   * Sends a message to a queue.
   * <P>
   * Send does not support deliveryMode, priority, and timeToLive. It will ignore anything in
   ...
   * @throws MessageFormatException If an invalid message is specified.
   * @throws UnsupportedOperationException If a client uses this method with a MessageProducer that
   *         did not specify a destination at creation time.
   * @throws JMSException If session is closed or internal error.
   */
  @Override
